...
class TradingSimulator:
    ...
    def get_backtest_filename(self, symbols, start_date_dt, end_date_dt, rebalance_frequency, long_count, short_count, portfolio_starting_value, risk_pct, reinvest_profits_bool):
    ...
    def analyze_backtest(self, test_filename: str):
        test = pickle.load(open(test_filename, 'rb'))
        backtest_profits = test['backtest_profits']
        benchmark_returns = self.get_benchmark_returns(test['rebalance_periods'][0][0], test['rebalance_periods'][-1][1], test['rebalance_periods'], test['inputs']['portfolio_starting_value'])
