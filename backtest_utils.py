...
from analysis_utils import BackTestManager
...
class TradingSimulator:
    def __init__(self, strategy_name):
        ...
        # Dynamically import a strategy based on the strategy name
        strategy_module = __import__('vault.' + strategy_name, fromlist=[''])
        self.strategy = strategy_module.Strategy()
    ...
    def run_backtest(self, symbols, start_date_dt, end_date_dt, rebalance_frequency, long_count, short_count, portfolio_starting_value, risk_pct, reinvest_profits_bool, leverage_multiplier, save_test: bool=False):
        ...
        with tqdm(total=len(rebalance_periods)) as pbar:
            ...
            for rebal_interval in rebalance_periods:
                ...
                # Run the trades
                if len(trading_data_interval.keys()) > 0:
        ...
        if save_test:
            ...
            self.backtest_manager.save_test(test)
