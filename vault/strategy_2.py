...
class Strategy (BaseStrategy):
    ...
    def generate_signals (self, market_data_df):
        ...
        for symbol in set(market_data_df["Open"].columns):
            ...
            symbol_scores[symbol] = market_data_df.loc[self.granularity].xs(symbol, axis=1, level='Ticker').iloc[-1]['close'] / market_data_df.loc[self.granularity].xs(symbol, axis=1, level='Ticker').iloc[-200]['close']
        ...
        ideal_portfolio['timestamp'] = market_data_df.loc[self.granularity].xs(symbol, axis=1, level='Ticker').iloc[-1]['Datetime'] # Find the latest timestamp.
        """
