...
class Strategy(BaseStrategy):
    def __init__(self):
        self.strategy_name = 'smooth_operator'
    
    def get_name(self):
        return self.strategy_name
    
    def get_pruned_data(self, df, days_ago_list):
        return df.iloc[-days_ago_list[-1]:] if len(df) >= days_ago_list[-1] else df
    
    def create_analysis_array_symbol(self, df, start_date_dt, days_ago_list):
        # Filter the dataframe to include only the data needed
        df_pruned = self.get_pruned_data(df, days_ago_list)
    ...
    def get_long_short_symbols(self, long_count, short_count, symbols_array, data_array, data_index):
        ...
        pct_change_count = pct_growth_count = 12
