...
class Strategy(BaseStrategy):
    def __init__(self, config_dict=None):
        ...
        self.trail_step_pct = 0.005  # Trail step for profit locking
        self.atr_period = 30  # ATR lookback period
        self.atr_multiplier = 4.0  # ATR multiplier for stops
        self.pullback_atr_ratio = 1.0  # Required ATR pullback for entry
        ...
        self.volume_multiplier = 0.6  # Required volume above SMA
    ...
    def generate_signals(self, next_rows, market_data_df, system_timestamp, strategy_margins, open_signals=None):
        ...
        self.strategy_margins = strategy_margins
        ...
        for symbol in symbols:
            ...
            if abs(sum(rule_outputs)) >= 5:
                # self.rules_logger.info({'symbol':symbol, 'rule_outputs':rule_outputs, 'rule_names':rule_names})
                # self.rules_logger.info({'symbol':symbol, 'rule_inputs':rule_inputs[-1]})
    ...
    def calculate_metrics(self, market_data_df, symbol, market_regime):
        ...
        try:
            ...
            try:
                ...
                higher_tf_sma20 = higher_tf_close.rolling(window=20).mean()
                # Determine 12h trend direction
                higher_timeframe_trend = 1 if higher_tf_close.iloc[-1] > higher_tf_sma20.iloc[-1] else -1
            except:
    ...
    def calculate_position_details(self, current_price, atr):
        ...
        if self.strategy_margins and 'USD' in self.strategy_margins:
            self.funds_available = float(self.strategy_margins['USD'].get('buying_power_available', 0))
            if position_value > self.funds_available:
                return 0, stoploss_price
            else:
                self.strategy_margins['USD']['buying_power_available'] -= position_value
                
        # Calculate final position size
        position_size = position_value / current_price
        
        return position_size, stoploss_price, position_value
