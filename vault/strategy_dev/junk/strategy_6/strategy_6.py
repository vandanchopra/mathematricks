...
MODEL_SAVE_PATH = "/Users/vandanchopra/Vandan_Personal_Folder/CODE_STUFF/Projects/mathematricks/vault/strategy_dev/strategy_6/model_checkpoints"  # Directory to save model checkpoints
...
class MarketDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = np.asarray(x, dtype=np.float32)  # Ensure numeric type
        y = np.asarray(y, dtype=np.int64)   # Ensure integer type
...
def train_model(model, dataloader, criterion, optimizer, epochs, start_epoch=0):
    try:
      ...
      for epoch in range(start_epoch, epochs):
          ...
          if (epoch + 1) % 5 == 0:
              checkpoint_path = os.path.join(MODEL_SAVE_PATH, f"model_epoch_{time.time()}.pth")
...
if __name__ == "__main__":
    ...
    if os.path.exists(MODEL_SAVE_PATH):
        ...
        # Keep only the last 10 checkpoints, delete the rest
