...
from systems.utils import project_path, sleeper, load_symbols_universe_df
...
class Strategy(BaseStrategy):
    ...
    def datafeeder_inputs(self):
        ...
        self.tickers = self.get_list_of_symbols_to_trade(self.start_time, universe_size=self.universe_size)
    ...
    def get_list_of_symbols_to_trade(self, start_time, universe_size=100):
        '''
        ...
        def load_prices_data(stock_symbols_df):
            # Define the folder containing the price data
            prices_folder = '/Users/vandanchopra/Vandan_Personal_Folder/CODE_STUFF/Projects/mathematricks/db/data/yahoo/1d'
            # make a list of symbols not in 'self.prices_data_dict'
            data_to_load_symbols_list = []
            for symbol in stock_symbols_df['Symbol']:
                if symbol not in self.prices_data_dict:
                    data_to_load_symbols_list.append(symbol)
            
            for symbol in tqdm(data_to_load_symbols_list, desc="Loading Price Data of Tickers to Trade: {}".format(self.strategy_name)):
                try:
                    price_data = pd.read_csv(os.path.join(prices_folder, f'{symbol}.csv'), parse_dates=['datetime'], index_col='datetime')
                    self.prices_data_dict[symbol] = price_data
                except:
                    pass
                    
            return self.prices_data_dict
        def calculate_growth_rate(stock_symbols_df, prices_data_dict):
            ...
            # Iterate over each symbol in the stock_symbols_df
            for symbol in tqdm(stock_symbols_df['Symbol'], desc="Creating List of Tickers to Trade: {}".format(self.strategy_name)):
                # Load the price data for the symbol
                try:
                    price_data = prices_data_dict[symbol]
                    # Prune data to only reflect the lookback period
                    price_data_pruned = price_data.loc[lookback_start_date:test_start_date]
                    # Get the latest price
                    one_year_ago_prices[symbol] = price_data_pruned.iloc[0]['adj close']
                    latest_price = price_data_pruned.iloc[-1]['adj close']
                    latest_prices[symbol] = latest_price
                    # self.logger.debug({'first_timestamp':price_data_pruned.iloc[0].name, 'last_timestamp':price_data_pruned.iloc[-1].name})
                except Exception as e:
                    # If there's an error (e.g., file not found or date not available), append NaN
                    latest_prices[symbol] = float('nan')
                    one_year_ago_prices[symbol] = float('nan')
                    # raise Exception(f'Error for symbol {symbol}: {e}')
            
            # Add the prices to the dataframe
            stock_symbols_df['latest_price'] = stock_symbols_df['Symbol'].map(latest_prices)
            stock_symbols_df['one_year_ago_price'] = stock_symbols_df['Symbol'].map(one_year_ago_prices)
            # Drop any rows where the latest price is NaN or one year ago price is NaN
            stock_symbols_df = stock_symbols_df.dropna(subset=['latest_price', 'one_year_ago_price'])
            # Now calculate the growth rate for each symbol
            # stock_symbols_df['growth_rate_pct'] = ((stock_symbols_df['latest_price'] - stock_symbols_df['one_year_ago_price']) / stock_symbols_df['one_year_ago_price']) * 100
            stock_symbols_df['growth_rate_pct'] = (((stock_symbols_df['latest_price'] - stock_symbols_df['one_year_ago_price']) / stock_symbols_df['one_year_ago_price']) * 100)
            # Round the growth rate to 2 decimal places
            stock_symbols_df['growth_rate_pct'] = stock_symbols_df['growth_rate_pct'].round(2)
            # Now sort by growth rate in descending order
        ...
        def get_top_bottom_symbols(stock_symbols_df, universe_size):
            ...
            top_bottom_n_symbols = list(top_bottom_n['Symbol'].values)
        ...
        stock_symbols_df = load_symbols_universe_df()
        # self.logger.debug({'stock_symbols_df':stock_symbols_df})
        ...
        prices_data_dict = load_prices_data(stock_symbols_df)
        
        stock_symbols_df = calculate_growth_rate(stock_symbols_df, prices_data_dict)
        # self.logger.debug({'stock_symbols_df':stock_symbols_df})
    ...
    def run_strategy(self, next_rows, market_data_df, system_timestamp, long_short_symbol_count=5):
        ...
        if self.granularity in next_rows.index and self.granularity in market_data_df.index.levels[0] and (len(market_data_df.loc[self.granularity]) > self.data_inputs[self.granularity]['lookback']):
            ...
            tickers_temp = list(set(self.tickers) & set(market_data_df.columns.get_level_values(1)))
            ...
            '''STEP 3: Calculate growth rate 1 on strategy dataframe'''
            ...
            '''STEP 4: Calculate growth rate 1 on strategy dataframe. Which is everything divided by the first row'''
            # Calculate the growth rate for each row
            ...
            weights_for_scores_combination = [20, 10]
    ...
    def generate_signals(self, next_rows, market_data_df, system_timestamp):
        ...
        if system_timestamp > self.config_dict['backtest_inputs']['start_time']:
            if self.days_since_rebalance >= self.rebalance_frequency:
                ...
                if self.tickers != tickers_new:
                    self.tickers = deepcopy(tickers_new)
                    # self.logger.debug(f"Tickers for {self.strategy_name} updated to: {self.tickers}")
