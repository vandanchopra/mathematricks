...
class Strategy(BaseStrategy):
    ...
    def return_notes(self):
        ...
        - 5_2: Vectorized, so backtesting should be faster
    ...
    def get_entry_indicator(self, df_1d, shortlen, longlen, basisLen, offset, offsetSigma):
        ...
        # Perform vectorized weighted moving average for 'close' and 'open' prices
        closeSeries = self.computed_prices(df_1d['close'], length=basisLen, offset=offset, sigma=offsetSigma)
        openSeries = self.computed_prices(df_1d['open'], length=basisLen, offset=offset, sigma=offsetSigma)
    ...
    def run_strategy(self, next_rows, market_data_df, system_timestamp):
        if self.granularity in next_rows.index and self.granularity in market_data_df.index.levels[0] and (len(market_data_df.loc[self.granularity]) > self.data_inputs[self.granularity]['lookback']):
            ...
            self.long_short_symbol_count = min(self.long_short_symbol_count, len(trend_strength_scores)/4)
            ...
            self.logger.debug({'long_symbols':long_symbols})
            self.logger.debug({'short_symbols':short_symbols})
            sleeper(5, 'LONG SHORT SYMBOLS')
            ...
            for count in range(0, max_required_len):
                for direction in ['LONG', 'SHORT']:
                    if direction == 'LONG':
                        symbol = long_symbols[count]
                    elif direction == 'SHORT':
                        symbol = short_symbols[count]
                    else:
                        continue
                    trend_score_value = trend_strength_scores.loc[symbol, 'final_score']
                    ...
                    if entry_indicator == 1 and direction == 'LONG':
                        long_signal_symbols[symbol] = {'direction':entry_indicator, 'trend_indicator_strength': trend_score_value, 'entry_indicator_strength':entry_indicator_strength, 'symbol_ltp':symbol_ltp}
                    elif entry_indicator == -1 and direction == 'SHORT':
                        short_signal_symbols[symbol] = {'direction':entry_indicator, 'trend_indicator_strength': trend_score_value, 'entry_indicator_strength':entry_indicator_strength, 'symbol_ltp':symbol_ltp}
            ...
            return long_signal_symbols, short_signal_symbols, decision_matrix
    ...
    def generate_signals(self, next_rows, market_data_df, system_timestamp, open_orders):
        ...
        if self.granularity in next_rows.index and self.granularity in market_data_df.index.levels[0] and (len(market_data_df.loc[self.granularity]) > self.data_inputs[self.granularity]['lookback']):
            if system_timestamp > self.config_dict['backtest_inputs']['start_time']:
                ...
                start = time.time()
                long_signal_symbols, short_signal_symbols, decision_matrix = self.run_strategy(next_rows, market_data_df, system_timestamp)
