...
class Strategy(BaseStrategy):
    def __init__(self, config_dict):
        ...
        self.min_market_cap_in_billions = 10
        self.tickers = []
    ...
    def run_strategy(self, next_rows, market_data_df, system_timestamp, long_short_symbol_count=5):
        ...
        if self.granularity in next_rows.index and self.granularity in market_data_df.index.levels[0] and (len(market_data_df.loc[self.granularity]) > self.data_inputs[self.granularity]['lookback']):
            ...
            # self.logger.debug({'strategy_df_orig':strategy_df_orig})
            ...
            # self.logger.debug({'strategy_df':strategy_df})
            ...
            # self.logger.debug({'growth_rate_1_df':growth_rate_1_df})
            growth_rate_1_df_normalized_scores = growth_rate_1_df.apply(lambda x: (x - np.min(x)) / (np.max(x) - np.min(x)), axis=1)
            # self.logger.debug({'growth_rate_1_df_normalized_scores':growth_rate_1_df_normalized_scores})
            ...
            # self.logger.debug({'growth_rate_2_df':growth_rate_2_df})
            ...
            # self.logger.debug({'growth_rate_2_df_normalized_scores':growth_rate_2_df_normalized_scores})
            ...
            # self.logger.debug({'scores_1_df':scores_1_df, 'scores_2_df':scores_2_df})
            ...
            # self.logger.debug({'scores_df':scores_df})
            final_scores = pd.DataFrame(scores_df.sum(axis=0))
            # self.logger.debug({'final_scores':final_scores})
        ...
        if system_timestamp > self.config_dict['backtest_inputs']['start_time']:
            if self.days_since_rebalance >= self.rebalance_frequency:
                ...
                if len(top_symbols) != 0 or len(bottom_symbols) != 0:  
                    ...
                    self.logger.debug({'top_scores:':top_scores, 'bottom_scores':bottom_scores})
