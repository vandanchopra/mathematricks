...
class DataFetcher:
    ...
    def fetch_price_data(self, start_date, end_date, lookback, throttle_secs=1, update_data=False, run_mode=4, live_bool=False, forward_update=True, backward_update=False):
        ...
        update_data = True if live_bool else update_data
        ...
        if data_source == 'yahoo':
            market_data_df = self.broker.sim.data.fetch_price_data(
                list_of_symbols, interval_inputs=interval_inputs, throttle_secs=throttle_secs,
                start_date=start_date, end_date=end_date, lookback=lookback,
                update_data=update_data, run_mode=run_mode,
                forward_update=forward_update, backward_update=backward_update)
            
        elif data_source == 'ibkr':
            market_data_df = self.broker.ib.data.fetch_price_data(
                list_of_symbols, interval_inputs=interval_inputs, throttle_secs=throttle_secs,
                start_date=start_date, end_date=end_date, lookback=lookback,
                update_data=update_data, run_mode=run_mode)
        elif data_source == 'kraken':
            market_data_df = self.broker.kraken.data.fetch_price_data(
                list_of_symbols, interval_inputs=interval_inputs, throttle_secs=throttle_secs,
                start_date=start_date, end_date=end_date, lookback=lookback,
                update_data=update_data, run_mode=run_mode)
        else:
